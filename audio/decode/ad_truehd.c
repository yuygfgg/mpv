...
#include "audio/decode/ad_starmine_common.h"
                              ...
                              struct mp_frame *out)
{
    ...
    return starmine_build_frame(ad, ctx->codec, ctx->pool, &frame, pts, out);
}

static int receive_renderer_flush_frame(struct mp_filter *ad, struct mp_frame *out)
{
    struct priv *ctx = ad->priv;
    starmine_ad_render_714_frame frame;
    starmine_ad_status status;
    ...
    if (ctx->renderer_flushed) {
        ctx->draining = false;
        return AVERROR_EOF;
    }

    if (starmine_ad_render_714_frame_init(&frame) != STARMINE_AD_STATUS_OK) {
        MP_ERR(ad, "failed to initialize Starmine flush frame\n");
        return AVERROR(EINVAL);
    }
    ...
    duration = starmine_render_frame_duration(&frame);
    if (pts != MP_NOPTS_VALUE && duration > 0.0)
        ctx->next_pts = pts + duration;
